...
const ByobProductsTable = ({ qrCodes }) => (
  <IndexTable
    resourceName={{
      singular: "QR code",
      plural: "QR codes",
  ...
  >
    {qrCodes.map((qrCode) => (
      <ByobProductRow key={qrCode.id} qrCode={qrCode} />
    ))}
  </IndexTable>
);

const ByobProductRow = ({ qrCode }) => (
...
export default function Index() {
  const { byobProducts } = useLoaderData();
  ...
  return (
    <Page>
      ...
      <Layout>
        <Layout.Section>
          <Card padding="0">
            {byobProducts.length === 0 ? (
              <EmptyByobProductsState onAction={() => navigate("byob/new")} />
            ) : (
              <ByobProductsTable qrCodes={byobProducts} />
            )}
          </Card>
        </Layout.Section>
      </Layout>
